...
def removeextinct(arbre):
	...
	finalSize = len(arbre)
	diffInSize = (initialSize-finalSize)
	print  str(diffInSize) + "extinct species have been removed."
	return arbre
...
#WE compute teh coordonates of FECA (First universal Common ancestor
feca = midpoint(-6.0,9.660254-10.0,0,-4.226497)


###################################################################
## UNCOMMENT WHAT FOLLOWS TO REMOVE THE BRANCH FROM LECA TO LUCA ##
###################################################################

##we add the way from LUCA to the root of the subtree when we are not in eukaryotes
...
# ##we add a way from the root of LECA to FECA 	
# if (groupnb=="2"):
# 	ndid=ndid+1
# 	command = "INSERT INTO lines (id, branch, zoomview, ref, way) VALUES(%d,'TRUE', '4','%s',ST_Transform(ST_GeomFromText('LINESTRING(%.20f %.20f, %.20f %.20f)', 4326), 900913));" % (ndid, groupnb, feca[0], feca[1], t.x, t.y);
# 	cur.execute(command);
# 	conn.commit()
...
##HERE WE CAN ADD A NICE DRAWING OF A BEZIER CURVE CONNECTING THE ROOT OF EUK TO THE MIDDLE BRANCH OF ASGARD
##THIS IS DONE WHEN WE ARE INSIDE THE ARCHAEA SUBTREE
#(x0,y0) = coordinates x of Euks
#(x3,y3) = coordinates of arch
#(x1,y1) = coordinates of luca
#(x2,y2) = coordinates of asgaard
if (groupnb=="1"):
	asgard = t&1935183
	archae = t
	archaemidpoint = midpoint(asgard.x,asgard.y,archae.x,archae.y)

	def bezier(X,Y,Nb):
		Xbezier = []
		Ybezier = []
		for t in np.linspace(0,1,Nb):
			Xbezier.append((1-t)**3*X[0] + 3*(1-t)**2*t*X[1] +3*(1-t)*t**2*X[2] + t**3*X[3])
			Ybezier.append((1-t)**3*Y[0] + 3*(1-t)**2*t*Y[1] +3*(1-t)*t**2*Y[2] + t**3*Y[3])
		return([Xbezier, Ybezier])

	# X = [-6.0, 0, asgard.x, asgardmidpoint[0]]
	# Y = [9.660254-10.0, -4.226497, asgard.y, asgardmidpoint[1]]
	X = [feca[0], 0, asgard.x, archaemidpoint[0]]
	Y = [feca[1], -4.226497, asgard.y, archaemidpoint[1]]
	CoordinatesBezierBranch = bezier(X,Y,100)
	##we add this NEW bezier branch in the tree 
	ndid=ndid+1
	cooLine = 'LINESTRING(%.20f %.20f' % (CoordinatesBezierBranch[0][0], CoordinatesBezierBranch[1][0]);
	for i in range(1,100):
		cooLine += ',%.20f %.20f' % (CoordinatesBezierBranch[0][i], CoordinatesBezierBranch[1][i]);
	cooLine += ')';
	command = "INSERT INTO lines (id, branch, zoomview, ref, way) VALUES(%d,'TRUE', '4','4',ST_Transform(ST_GeomFromText('%s', 4326), 900913));" % (ndid, cooLine);
	##WE CREATE A NEW 'ref' that will allow us to filter this curve exactly
	cur.execute(command);
	conn.commit()


##SIMILARLY WE CAN ADD A BRANCH CONNECTING alphaproteobacteria to euk
##THIS IS DONE WHEN WE ARE INSIDE THE BACTERIA SUBTREE
#(x0,y0) = coordinates x of Euks
#(x3,y3) = coordinates of proteobacteria
#(x1,y1) = coordinates of luca
#(x2,y2) = coordinates of alphaproteobacteria
if (groupnb=="3"):
	alphaproteo = t&28211
	proteo = t&1224
	proteomidpoint = midpoint(alphaproteo.x,alphaproteo.y,proteo.x,proteo.y)
#	asgardmidpoint = midpoint(asgard.x,asgard.y,t.x,t.y)
	...
	# X = [-6.0, 0, asgard.x, asgardmidpoint[0]]
	# Y = [9.660254-10.0, -4.226497, asgard.y, asgardmidpoint[1]]
	X = [feca[0], 0, alphaproteo.x, proteomidpoint[0]]
	Y = [feca[1], -4.226497, alphaproteo.y, proteomidpoint[1]]
	CoordinatesBezierBranch = bezier(X,Y,100)
	##we add this NEW bezier branch in the tree 
	ndid=ndid+1
	cooLine = 'LINESTRING(%.20f %.20f' % (CoordinatesBezierBranch[0][0], CoordinatesBezierBranch[1][0]);
